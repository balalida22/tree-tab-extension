...
- `web-ext` lint completed with no errors, notices, or warnings, and the version 1.1.1 archive passed integrity and content checks.
- A live Firefox or in-app browser was unavailable, so nesting/reordering pointer gestures and the sub-forest group dialog were not exercised against a running sidebar. The local README contains the manual Firefox verification steps.
